...
### Fixed
- Draft summary reports write workspace-relative SSP paths instead of machine-specific absolute paths
- Plugin and marketplace manifests identify this independent repository as owner instead of implying oscal-compass affiliation; README disclaimer extended to cover oscal-compass and CNCF
- Claude Code marketplace install command in the compatibility matrix points at this repository
- README adds installation instructions and current version reference

## [0.2.1] - 2026-07-21

### Added
- `validate-oscal-package.sh` runs `oscal-cli` NIST schema validation directly when it is on `PATH` (constraint validation disabled because Trestle-internal `trestle://` hrefs are not resolvable outside Trestle)
- Draft SSP test suite schema-validates the generated catalog, profile, and SSP with `oscal-cli` when available
- CI installs Temurin Java 21 and `oscal-cli` 3.2.0 and runs the Cursor support suite for full end-to-end coverage
- Validation layers documented in `docs/OSCAL-DOCUMENT-WORKBENCH.md`; `oscal-cli` listed as an optional prerequisite in the README

### Fixed
- Draft SSP generator no longer emits empty `metadata.roles`, `params`, `props`, or `links` arrays, which violated OSCAL minimum-item schema constraints flagged by `oscal-cli`
- Source map CSV writers emit LF line endings instead of CRLF
- Demo Trestle workspace output is regenerated by `full-demo.sh` and no longer tracked in git
